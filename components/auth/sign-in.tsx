...
const SignInForm = () => {
  ...
  const urlError =
    searchParams.get("error") === "OAuthAccountNotLinked"
      ? "Email is aleady in use with a different account!"
      : "";
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {!showTwoFactor && (
            <>
              ...
              <div className="w-full mb-7">
                <Link
                  href="/auth/reset"
                  className="text-sm font-medium hover:underline hover:text-primary ease-out duration-[0.3s]"
                >
                  Forgot password?
                </Link>
              </div>
          ...
          <button
            ...
            disabled={pending}
          >
            {pending ? <BeatLoader /> : showTwoFactor ? "Confirm" : "Sign In"}
          </button>
        </form>
      </Form>
    </AuthCard>
