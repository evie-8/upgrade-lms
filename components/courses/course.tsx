...
const Courses = ({ courses }: Props) => {
  ...
  const [selectedSkillLevel, setSelectedSkillLevel] = useState<string | number>(
    "All"
  );
  ...
  const filterCourses = () => {
    ...
    if (selectedCategory && selectedCategory !== "All") {
      newData = newData.filter(
        (course: any) => course.category.name === selectedCategory
      );
    }

    if (selectedSkillLevel && selectedSkillLevel !== "All") {
      newData = newData.filter(
        (course) => course.difficulty === selectedSkillLevel
      );
    }

    if (selectedPrice && selectedPrice !== "All") {
      newData = newData.filter(
        (course) => course.paymentStatus === selectedPrice
      );
    }
    ...
    <section className="relative">
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url(images/courses.jpg)`,
      ...
      <Container>
        <div className="all-courses-container">
          <nav
            className={` ${sideBar ? "right-0" : "-right-[100%]"} ${
              show ? "lg:hidden" : "show"
            }`}
            tabIndex={0}
            ...
            <div className="sidebar">
              ...
              <div className="title-area">
...

                {
                  <button
                    onClick={handleClick}
                    className="hidden lg:flex items-center sm:hidden"
...

                {
                  ...
                  <button
                    ...
                    className="flex sm:flex items-center lg:hidden"
                    title="hide filter sidebar"
                  >
                    <SidebarClose className="text-primary" />
                  </button>
                    ...
                    <CourseCardList
                      ...
                      cost={
                        course.paymentStatus === "Free"
                          ? "Free"
                          : formatter(course.price?.toString())
                      }
                    />
                  ))}
              </section>
      ...
      <Banner
        ...
        link="/courses"
        title="Need help finding a course?"
        action="Find Your Programme"
