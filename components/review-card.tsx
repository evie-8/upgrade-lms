          ...
          {review?.reviewer.image ? (
            <div className=" flex  items-center justify-center w-12 h-12 rounded-full border-2 border-primary ">
              <Image
                ...
                height={48}
                alt="profile-image"
                className=" w-full h-auto rounded-full  m-1"
              />
            </div>
          ) : (
            <div
              className={`flex gap-3 items-center justify-center rounded-full bg-blue1 w-12 h-12 border border-transparent`}
            >
      ...
      <div className="flex gap-2 items-center mt-1">
        {[...Array(5)].map((_, index) => (
          <FontAwesomeIcon
            ...
            icon={faStar}
            className={`w-3 h-3  ${
              index < review?.rating ? "text-ranking" : "text-grey"
            }  `}
          />
        ))}
        <p className="text-sm font-semibold">{review?.ratingDescription}</p>
      </div>
      <p className="text-sm font-normal my-3">{review?.review}</p>
    </div>
  );
};
