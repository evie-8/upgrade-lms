...
import { getNumberOfUsersEnrolledInFreeCourse } from "@/action-server/get-count-courses";

interface Props {
  resources: Resource[];
  purchase: Order | null;
  course: Course & { category: Category } & { tutor: User } & {
    orders: Order[];
...
const LessonView = ({
  ...
  const [enrolledFreeCourse, setEnrolledFreeCourse] = useState(0);
  ...
  const onvideoEnd = async () => {
    try {
      if (completeOnEnd) {
        await axios.put(
          `/api/courses/${courseId}/chapters/${lesson.chapterId}/lessons/${lesson.id}/progress`,
          {
            isCompleted: true,
          }
        );
      ...
      if (nextLesson) {
        router.push(
          `/student/courses/${courseId}/chapter/${nextLesson.chapterId}/lesson/${nextLesson.id}`,
          {},
  ...
  useEffect(() => {
    const getCount = async () => {
      if (course.paymentStatus === "Free") {
        const count = await getNumberOfUsersEnrolledInFreeCourse(courseId);
        setEnrolledFreeCourse(count);
      }
    };
      ...
      <div className="flex flex-col  gap-y-3">
        <div className="flex items-center justify-between gap-5 mb-3">
          <Button
            ...
            onClick={() =>
              router.push(
                `/student/courses/${courseId}/chapter/${prevLesson.chapterId}/lesson/${prevLesson.id}`,
                {},
                { showProgressBar: true }
              )
            }
        ...
        <div className="flex justify-between flex-wrap gap-4">
          <h2 className="text-xl font-semibold max-sm:basis-full">
            {lesson.name}
          </h2>
        ...
        <div className="flex gap-5 flex-wrap">
          <p className="text-sm">
            Course by{" "}
            <span className="font-semibold underline">{course.tutor.name}</span>
          </p>
          <p className="flex items-center justify-center gap-2">
            <FontAwesomeIcon icon={faStar} className="w-4 h-4 text-ranking" />
            <span className="font-semibold text-sm">
              {course.reviews.length
                ? (reviewSum / course.reviews.length).toPrecision(2)
                : 0}
          ...
          <p className="flex items-center justify-center gap-3">
            ...
            {course.paymentStatus === "Free" && (
              <span className="font-medim text-sm">
                {enrolledFreeCourse}{" "}
                {enrolledFreeCourse == 1 ? "student" : "students"}
              </span>
            )}
            ...
            {course.paymentStatus === "Paid" && (
              <span className="font-medim text-sm">
                ...
                {course.orders.length == 1 ? "student" : "students"}
              </span>
            )}
          </p>
        </div>
        ...
        <div className="flex items-start justify-start gap-3 flex-wrap">
          {buttons.map((button) => (
            <Button
              onClick={() => setClick(button.title)}
              key={button.title}
              variant={click ? "default" : "outline"}
              className={`${
                click !== button.title &&
                "bg-white text-gray border-white hover:bg-white/90"
              } `}
            >
                ...
                <div className="ql-container ql-bubble">
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: String(course.description),
                    }}
                  />
                </div>
              </div>
              ...
              <div className="quill">
                <div className="ql-container ql-bubble">
                  <div
                    className="desc mt-2 ql-editor  !leading-[1.5] !text-[16px]"
                    dangerouslySetInnerHTML={{
                      __html: String(lesson.chapter.description),
                    }}
                  />
                </div>
              </div>
            ...
            (resources.length > 0 ? (
              <div>
                ...
                {resources.map((resource) => (
                  ...
                  >
                    ...
                    <p className="line-clamp-1 text-sm">{resource.name}</p>
                  </a>
                ))}
              </div>
            ) : (
              <div>
                <h2 className="font-bold">
                  No material was provided for this course
                </h2>
              </div>
            ))}
          {click === "Reviews" && (
